...
You may notice in your data folder that the GBIF

::: {.callout-task title="Import GBIF Data"}

1. Change `'gbif-filename-here'` to match the filename of the GBIF data you downloaded ()
2. Load the data in with an appropriate library for the file extension.
3. Run the cell
:::

```{python}
#| template: student
# Get a path to the GBIF data
gbif_path = project.data.get_data_path('gbif-filename-here')
# Load the GBIF data
...
#| template: answer
# Get a path to the GBIF data
gbif_path = project.data.get_data_path(gbif_filename)
# Load the GBIF data
