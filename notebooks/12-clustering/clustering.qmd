...
We recommend building a GeoDataFrame, as this will allow you to plot the granules you are downloading and make sure they line up with your shapefile. You could also use a DataFrame, dictionary, or a custom object to store this information.
...
def compute_reflectance_da(search_results, boundary_gdf):
    ...
    def open_dataarray(url, boundary_proj_gdf, scale=1, masked=True):
        # Retry transient server-side failures from Earthdata/fsspec
        max_retries = 5
        retry_delay = 1
        errs_to_retry = [
            '502', '503', '504', 'Bad Gateway', 'ClientResponseError']
        ...
        for attempt in range(max_retries):
            try:
                ...
                transient_error = any(
                    text in str(err)
                    for text in errs_to_retry
                )
                if (not transient_error) or attempt == max_retries - 1:
                    raise
                print(
                    f'Raster open failed with a transient HTTP error. '
