...
description: |
  ...
  area.
learning-goals:
  - "Understand how to access and harmonize open datasets for environmental analysis."
  - "Process and visualize daily ozone and temperature data for an urban area."
jupyter:
  kernelspec:
...
The AirNow database from the Environmental Protection brings together a wide network of qir quality monitoring stations from across the United States. These air quality sensors provide valuable real-time information on pollutants such as ozone. However, they are sited irregularly around the country, which presents a challenge when comparing them to uniform gridded datasets from satellite platforms or reanalysis products. In this vignette, we will show how to harmonize the AirNow data with gridded Daymet temperature data in an urban area.

## Setup

First, we need to import packages and set up directories to store data for the project.
...
We will use the US Census data to define the urban area of interest, in this case Denver, CO.
...
### Download and process ozone data

Next, we retrieve the daily ozone data from EPA AirNow using the AirNow API.
...
Next, the monitoring station metadata:
...
Merge the monitoring station data with the ozone:
...
And finally crop and summarize to daily:
...
Next, we retrieve the maximum daily temperature data from Daymet:
...
Get average values for the Denver area:
...
## Merge ozone and temperature data

Now that we have daily values for the spatial domain, we can run a single command to combine the data:
