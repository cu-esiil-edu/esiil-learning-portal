...
::: {.cell .markdown}
:::: {.callout-respond .column-margin title="Is linear OLS regression right for your data?"}
...
::: {.callout-task title="Regression"}
  ...
  1. To get sample code, ask ChatGPT how to fit a linear model to your data. If you're new to using large language models, go ahead and check out [our query](https://chatgpt.com/share/649b897b-9075-457e-8e12-308f795312a1)
...
# Drop no data values
observations = ann_climate_df.temp_f.dropna()
...
::: {.callout-important .column-margin title="Is it ok to plot a trend line even if OLS isn't an appropriate model?"}
...
::: {.callout-task title="Regression Plot"}
  ...
  3. Can you figure out how to customize the colors and line style on your plot? Check out the [`seaborn` documentation](https://seaborn.pydata.org) for ideas.
...
ax.set(
    title=(
        'Annual Average Daily Temperature over time'
        f' in {location}'),
    xlabel='Year',
    ylabel='Temperature ($^\circ$F)'
)
