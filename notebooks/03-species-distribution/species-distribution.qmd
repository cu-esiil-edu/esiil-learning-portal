...
Before your get started, go to <https://www.gbif.org/> to explore the data!
...
1.  Look at the beginning of the file you downloaded. What do you think the **delimiter** is?
2.  Run the next code cell. What happens?
3.  Uncomment and modify the parameters below until your data loads successfully and you have only the columns you want.
:::
...
Use the following code to plot your data. You may need to modify it to get it to work!

::: {.content-visible when-format="html"}
```{python}
# Prepare a GeoDataFrame for plotting
occurrence_gdf = (
    # Start with the simplified plotting GeoDataFrame 
    ecoregions_plot_gdf
    # Join with the observation counts
    .join(occurrence_df)
    # Sort by month so the month names will be in the right order
    .sort_values('month')
)
