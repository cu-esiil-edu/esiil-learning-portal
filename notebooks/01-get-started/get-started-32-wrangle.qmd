...
Next, lets download some climate data from {{< meta params.01-get-started.location >}} to practice with. We keep our practice data on GitHub, so that we can check that it still works and make sure it looks just like the data you would download from the original source.
...
(
    ...
    '/releases/download/data-release/get-started-test-data.csv'
)
```
