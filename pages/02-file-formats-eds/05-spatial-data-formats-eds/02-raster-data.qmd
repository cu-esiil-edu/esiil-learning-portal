...
description: |
  Learn fundamental concepts related to working with raster data in **Python**, including understanding the spatial attributes of raster data, how to open raster data, and how to visually plot the data. 
learning-goals:
...
![A raster is composed of a regular grid of cells. Each cell is the same size in the x and y direction. Source: Colin Williams, NEON.](/img/earth-analytics/raster-data/raster-concept.png)


::: {.callout-read}
For more information on rasters, how they work, and the types of data stored in rasters, see [this chapter](https://www.earthdatascience.org/courses/use-data-open-source-python/intro-raster-data-python/fundamentals-raster-data/) on using raster data from the earth data science intermediate textbook.
:::
...
::: {.callout-read}
Learn more about working with GeoTiff files in <a href="https://www.earthdatascience.org/courses/use-data-open-source-python/intro-raster-data-python/fundamentals-raster-data/intro-to-the-geotiff-file-format/" target="_blank">this earth data science textbook lesson.</a>. 

Learn more about working with HDF4 files (the format used to store MODIS data) in <a href="https://www.earthdatascience.org/courses/use-data-open-source-python/hierarchical-data-formats-hdf/intro-to-hdf4/" target="_blank">this earth data science textbook chapter.</a>
:::
...
data_dir = et.data.get_data("colorado-flood")
data_dir
```
...
::: {.callout-info}
To view the numpy array stored inside of an xarray DataArray, you can access it by putting `.values` at the end of your Rioxarray variable's name. For example, `rioxarray_name.values` will return a numpy array with all the values stored in it. We use `.values` to plot with the earthpy `plot_rgb` function, since that needs a numpy array as an input!
:::
...
## Challenge:  Open & Plot a Raster Dataset
...
lidar_dem_path_post_flood = os.path.join(
    "data", "colorado-flood", "spatial",
    "boulder-leehill-rd", "post-flood", "lidar",
    "post_DTM.tif")
```

::: {.callout-info}
Don't forget to use `rxr.open_rasterio()` and assign the output to a variable! Also, don't forget to call `.values` on your rioxarray when plotting it with `plot_rgb`, that function needs a numpy array input to work!
:::

An example of what your plot should look like is below. 

```{python}
#| caption: 'Challenge Plot: Lidar Digital Terrain Model'
#| tags: [hide]
...
::: {.callout-read}
NAIP data is imagery collected by the United 
...
#| lines_to_next_cell: 2
# Download NAIP data
naip_data = et.data.get_data(url="https://ndownloader.figshare.com/files/23070791")
```
...
::: {.callout-task}
Next, make a plot comparing pre- and post-fire
:::
